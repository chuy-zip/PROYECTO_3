...
# Filtrar el dataset
#data <- data %>%filter(Caudef %in% top_10_causas)
...
candidata_top_letra <- data %>%
  filter(causa_macro == "Candidata", Caudef != "Otras") %>%   # <--- agregar esto
...
data <- data %>%
  mutate(
    causa_macro = case_when(
      ...
      letra_inicial == candidata_top_letra ~ "Síntomas y causas mal definidas",
...
```{r drownsampling_macro, echo=FALSE}
library(dplyr)
library(knitr)

# 1. Calcular el tamaño base igual al mínimo entre las 5 categorías
min_n <- data %>%
  count(causa_macro) %>%
  pull(n) %>%
  min()

cat("El número mínimo de casos entre las 5 categorías es:", min_n, "\n")

# 2. Aplicar submuestreo para que todas las categorías tengan igual número de registros
...
data_balanceada %>%
  count(causa_macro) %>%
  mutate(pct = round(100 * n / sum(n), 2)) %>%
  kable(caption = "Distribución balanceada de las 5 macro-categorías (downsampling aplicado)")


```

```{r to_factor, echo=FALSE}
# Convertir la variable 'causa_macro' a factor
data_balanceada$causa_macro <- as.factor(data_balanceada$causa_macro)

# Verificar estructura
str(data_balanceada$causa_macro)

# Visualizar niveles
levels(data_balanceada$causa_macro)


```

```{r export_data}
# Exportar el dataset balanceado a un CSV
write.csv(data_balanceada, "defunciones_balanceadas.csv", row.names = FALSE)

cat("Archivo 'defunciones_balanceadas.csv' exportado correctamente.")
